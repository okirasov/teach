...
Пользовательских предметов может быть сколько угодно. Стор (`src/store/progress.ts`) хранит их по id: `subjects`, `lessons` (текущий урок каждого), `prep` (этап и ошибка подготовки каждого) — раньше это были одиночные `custom`/`customLesson`/`prepStage`/`prepError`. Порядок на экранах — по `createdAt` (`userSubjectIds`). Id первого предмета остаётся историческим `custom`, следующие получают `custom-2`, `custom-3`: карточки повторов и справочники в SQLite уже привязаны к этому id, поэтому данные тестировщиков переживают обновление. Снимок kv старой версии приводит к новой форме `migrateProgressSnapshot`, подключённый как `migrate` в `persistSlice`.
