...
- `Endpoints/TalkEndpoints` — `POST /subjects/{id}/talks`, `POST /talks/{id}/turns` (SSE: `delta`…, `done`, `error`), `POST /talks/{id}/end`.
- `Jobs/DigestService` — собирает дайджест предмета (`SubjectRow.DigestText`) после урока, если его ещё нет; используется и воркером, и разговором.
- `Domain/TalkHistory` — окно в 10 последних реплик разговора; старше — сворачивается в `OlderSummary`.

## Разговор с бадди

Голосовой разговор по теме предмета (FRD FR-66…68, SR-16). `POST /subjects/{id}/talks` открывает разговор, `POST /talks/{id}/turns` со стримингом по SSE:
клиент присылает распознанный текст, сервер отдаёт реплику бадди кусками (`delta`…, `done`), чтобы первое предложение озвучилось до конца генерации,
`POST /talks/{id}/end` завершает разговор. Промпт из трёх слоёв с `cache_control`: персона и правила бадди (общие для всех), дайджест предмета (общий для предмета),
хвост диалога. Дайджест на 1–2 тыс. токенов (тема, фокус, миссия, этап, глоссарий, свёртка записей) собирает `claude-haiku-4-5` задачей воркера после каждого
разбора и хранит в строке предмета; уроки целиком в контекст не идут. История разговора хранится на сервере: последние 10 реплик, старое сворачивается в две
фразы. Модель по умолчанию `claude-sonnet-5` с низким effort, ответ устный на 1–3 предложения, `max_tokens` около 400 (с учётом thinking); ориентир — $0.003 за реплику, $0.05 за
разговор на 20 реплик. Бадди говорит на «ты». Следующий шаг: по окончании разговора извлекать записи (что подтверждено, где путался) и сохранять их как записи
разбора с пометкой «разговор».
