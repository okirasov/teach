...
После первого входа показывается интро (`app/intro.tsx`, четыре экрана про метод, свайп или «Дальше»; флаг `settings.introSeen` в kv аккаунта), затем мастер первого предмета, если предметов нет. Демо-предметов два: английский (языковой) и история (неязыковой, с шагами «порядок» и «свободный ответ»), список в `DEMO_IDS`; QA из прототипа остался в `seed.ts` только для тестов. Их карточки и справочники засеваются один раз на базу аккаунта (`seedDemo`, флаг `demo.seeded.v2` в kv); миграция v3 перед этим вычистила демо-данные старого набора. Вкладки без предметов показывают подсказку и кнопку «Новый предмет».
