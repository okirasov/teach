...
Озвучка (`src/voice/tts.ts`, `expo-speech`): системный синтез на устройстве, без сети и без токенов. Кнопка «динамик» (`SpeakButton`) у карточки «ПРИМЕР», у вариантов выбора, реплик для упорядочивания и правильного ответа; язык голоса — язык предмета, выключается в настройках предмета («Озвучка примеров»). Для неязыковых предметов и при отсутствии голоса на устройстве кнопок нет (`SpeakerProvider` проверяет `getAvailableVoicesAsync`). Смена шага и выход из сессии останавливают речь.
