...
После первого входа показывается интро (`app/intro.tsx`, четыре экрана про метод, свайп или «Дальше»; флаг `settings.introSeen` в kv аккаунта), затем мастер первого предмета, если предметов нет. Демо-предметы прототипа (английский, QA, история) в приложении больше не показываются: `activeSubjectIds` возвращает только пользовательский предмет, новая база не засевается карточками и справочниками прототипа, миграция v3 удаляет их из существующих баз. Данные `src/domain/seed.ts` остались для тестов и урока повторов. Вкладки без предметов показывают подсказку и кнопку «Новый предмет».
